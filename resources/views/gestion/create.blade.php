...
<h2>Nuevo Registro Gestión y Fomento Socioeconómico</h2>




<?php
	$nuevogestion = DB::table('registrosgestion')->max('numero');
?>
...
<form class="col-md-4" action="{{ url('gestion') }}" method="post">

	
	<input class="form-control" type="text" name="numero" value="<?php echo ($nuevogestion+1)?>" placeholder="Numero de Oficio" style="visibility:hidden">
	<h5>{{ ($errors->has('numero')) ? $errors->first('numero') : '' }} </h5>

	Numero de Oficio: P.SGFS<input type="text" name="date" class="form-control" value="<?php echo ($nuevogestion+1)?>" disabled><br>
	...
	<a href="{{ url('gestion') }}" class="btn btn-warning" style="width: 40%;">Regresar</a>
	</center>
