...
<h2>Nuevo Registro Acompañamiento Integral</h2>




<?php
	$nuevoaintegral = DB::table('registrosaintegral')->max('numero');
?>
...
<form class="col-md-4" action="{{ url('aintegral') }}" method="post">

	
	<input class="form-control" type="text" name="numero" value="<?php echo ($nuevoaintegral+1)?>" placeholder="Numero de Oficio" style="visibility:hidden">
	<h5>{{ ($errors->has('numero')) ? $errors->first('numero') : '' }} </h5>

	Numero de Oficio: P.SAI<input type="text" name="date" class="form-control" value="<?php echo ($nuevoaintegral+1)?>" disabled><br>
	...
	<a href="{{ url('aintegral') }}" class="btn btn-warning" style="width: 40%;">Regresar</a>
	</center>
