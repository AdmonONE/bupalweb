...
<h2>Nuevo Registro Cultura</h2>




<?php
	$nuevocultura = DB::table('registroscultura')->max('numero');
?>
...
<form class="col-md-4" action="{{ url('cultura') }}" method="post">

	
	<input class="form-control" type="text" name="numero" value="<?php echo ($nuevocultura+1)?>" placeholder="Numero de Oficio" style="visibility:hidden">
	<h5>{{ ($errors->has('numero')) ? $errors->first('numero') : '' }} </h5>

	Numero de Oficio: P.SCUL<input type="text" name="date" class="form-control" value="<?php echo ($nuevocultura+1)?>" disabled><br>
	...
	<a href="{{ url('cultura') }}" class="btn btn-warning" style="width: 40%;">Regresar</a>
	</center>
