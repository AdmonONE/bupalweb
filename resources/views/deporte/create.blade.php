...
<h2>Nuevo Registro Actividad Física y Deportiva</h2>




<?php
	$nuevodeporte = DB::table('registrosdeporte')->max('numero');
?>
...
<form class="col-md-4" action="{{ url('deporte') }}" method="post">

	
	<input class="form-control" type="text" name="numero" value="<?php echo ($nuevodeporte+1)?>" placeholder="Numero de Oficio" style="visibility:hidden">
	<h5>{{ ($errors->has('numero')) ? $errors->first('numero') : '' }} </h5>

	Numero de Oficio: P.SAFD<input type="text" name="date" class="form-control" value="<?php echo ($nuevodeporte+1)?>" disabled><br>
	...
	<a href="{{ url('deporte') }}" class="btn btn-warning" style="width: 40%;">Regresar</a>
	</center>
